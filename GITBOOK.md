...
## Add a course without editing JSON

`publisher/` is a FastAPI app with a form for exactly this: repository, branch, notes
directory, name and description, then **Check source** and **Push to GitBook**. It runs the
same pipeline (prune, import, regenerate, validate, commit, push) and never force-pushes.
See [PUBLISHER.md](PUBLISHER.md). The rest of this document is the command-line equivalent.
