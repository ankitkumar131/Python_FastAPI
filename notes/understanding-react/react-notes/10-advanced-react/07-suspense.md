...
`Suspense` is a **boundary that React can fall back to when a child cannot render yet.** A child signals "not ready" by _suspending_: it throws a promise (or, with `use()`, React observes the pending promise and suspends for you). React then:
...
Same shape as the lazy case in chapter 05, because it _is_ the same mechanism: React saw a pending promise, rendered the fallback, and swapped in the real content when it settled.
...
Where you would _not_ write this by hand:
...
`Suspense` handles _pending_; it has nothing to show for _failed_. The rejection is thrown during render, so it travels to the nearest **error boundary** (chapter 08). Three practical consequences:
...
The lab's `Map` of promises is a teaching device. Production code wants the features a data library already has: deduplication, caching, retries, invalidation, background refresh, and _both_ the flag-based and suspense-based APIs.
...
* **`useDeferredValue`/transitions** for expensive _rendering_ (file 09) are the rendering-side twin of Suspense: one defers waiting on data, the other defers work on data you already have.
...
| --------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| "Suspense fetches my data"                    | it renders a fallback while _something else_ fetches                                           |
...
| "Suspense replaces loading states everywhere" | it replaces _some_ of them, in the places where a boundary makes sense                         |
...
⚠️ **Do not mix patterns inside one data flow.** A screen that fetches with flags in one component, suspends in the next and prefetches with a hand-written `Map` in a third is a screen nobody can reason about. Pick per _screen_, prefer the library, and keep the suspense caches (if any) in one module.
...
1. **Think of `Suspense` as a rendering boundary**: it decides _where_ "not ready" shows, never _how_ data is fetched.
...
3. Explain and demonstrate the difference between `Suspense` and `useDeferredValue` for a slow list: modify the lab so the _data_ is suspenseful but the _filtering_ is expensive, and show which tool fixes which half. Measure both and write the paragraph that tells a teammate which to reach for.
