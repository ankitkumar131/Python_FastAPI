...
| 2        | `getByLabelText`              | form fields (a label _should_ exist)                 | `getByLabelText('Search products')`          |
...
`userEvent`'s methods already await the event and the resulting commits, so clicking is covered; what is _not_ covered is work that finishes later — a fetch, a `setTimeout`, a transition. Those need `findBy*` (preferred) or `waitFor` (for assertions that are not queries):
...
|                   | `fireEvent`                                                                     | `userEvent`                                            |
| ----------------- | ------------------------------------------------------------------------------- | ------------------------------------------------------ |
| What it does      | dispatches one DOM event                                                        | simulates the _sequence_ a real user produces          |
...
* **`toHaveTextContent('5')`** asserts the _outcome_ the user sees after an interaction, which is the highest-value observation in this component.
...
2. Write one assertion with `queryBy` proving something is _not_ rendered, and one with `findBy` that waits for text to appear.
