...
⚠️ **Route order does not decide protection** — the guard does. What matters is which routes are _inside_ the guarded layout route. Keep the guarded block as large as possible so new pages inherit it, and keep the public exceptions explicit and few.
...
1. Implement a `RequirePermission` component that supports role _and_ permission checks, renders an accessible 403 with a link back, and logs the denial (for product analytics) without leaking data. Test all branches.
...
* **Redirects have rules**: `replace` for auth navigations, keep the destination through login _and_ logout, do not redirect on background 401s, and prefer an honest 403 to a silent bounce.
