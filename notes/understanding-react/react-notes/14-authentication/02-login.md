...
⚠️ Do not `navigate` inside the provider. Redirecting is a _screen_ concern (it depends on where the user was), while authenticating is an _app_ concern. Keeping them apart is what makes the provider importable in tests without a router.
...
| Confirmation field | no                              | yes — validate equality _and_ decide what to do when one field changes (Part 8's `deps`)               |
