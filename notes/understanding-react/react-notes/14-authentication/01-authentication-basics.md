...
| 9    | leaving the token behind on logout      | clear storage _and_ state; revoke server-side if possible              |
...
💡 The way to state it in a review: _"This guard is a UX guard; the endpoint must enforce the same rule."_ If the endpoint does not, the PR is not ready.
