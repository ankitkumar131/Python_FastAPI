...
**Order matters:** the type-check and tests run _before_ deploy, on the same artifact that will be shipped (`npm ci` → build once → deploy that artifact). Rebuilding on the deploy machine is how "it passed in CI but the deployed bundle differs" happens, most often through environment variables (file 01).
...
| Readability   | is the _why_ explained where it is not obvious?                                                                |
...
| -- | ---------------------------------------------- | -------------------------------------------- | -------------------------------------------------- |
| 1  | No SPA fallback                                | deep links 404 (measured above)              | configure rewrites/try\_files                      |
