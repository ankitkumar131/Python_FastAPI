...
⚠️ **Re-renders are not the enemy.** A re-render is React re-running your component functions; if the output is the same, React touches nothing in the DOM. The cost is the _work_ inside your components and the _paint_ that follows, not the fact that a function ran. Optimising "the number of renders" without measuring is how teams add `memo` everywhere and ship a slower, more complex app.
...
💡 **The blast-radius question** is the fastest way to reason about performance: _when this state changes, which components re-render?_ If the answer is "the whole page", colocate or split before reaching for `memo`.
...
The measurement in section 2 is unambiguous: the cost scales with the _number of rendered rows_ (20 000 rows ≈ 1 s to mount, 374 ms per update), while `memo` only removes the per-row render work. Windowing renders a small slice:
