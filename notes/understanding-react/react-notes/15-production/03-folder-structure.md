...
The pragmatic rule used by most teams: **one component = one file** until it needs siblings, then promote it to a folder _without_ an `index.ts` unless the folder is a feature boundary:
...
💡 Add **one** alias (`@/`) rather than five. Fewer aliases means fewer configs to keep in sync and less ambiguity; if you need to _enforce_ boundaries, use lint rules (file 02, section 5) rather than alias names.
