...
**Origin = scheme + host + port.** `https://app.example.com:443` and `http://app.example.com` are different origins; so are `https://api.example.com` and `https://app.example.com`. The browser allows your page to _display_ cross-origin resources (images, scripts) but blocks your **JavaScript** from _reading_ cross-origin responses — that is the same-origin policy, and CORS is the server's way to say "these other origins may read my responses in a browser".
...
| --------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- | ----------------------------------------------------------------------- |
| **Simple** request (GET/POST with simple headers and content types)                                       | yes, then the browser checks the response                        | a blocked response still _reached_ the server — mutations can happen    |
...
⚠️ **CORS is not authorisation and not CSRF protection.** It restricts which _origins_ may read responses in a browser; it does nothing for a `curl` client, a server-to-server call, or a form POST from another site. The API must authenticate and authorise every request regardless.
...
⚠️ **Do not build CSRF protection by checking a header in the client.** The protection is the server rejecting requests without a valid token/origin; the client's job is to send the token (from a cookie your JS _can_ read, or from a meta tag) and to use `SameSite` cookies.
...
| "The route guard protects the page."             | ❌ it protects the _user experience_; data must be protected by the API       |
...
1. Build a small cross-origin request (a static page on one port, an API on another) and observe: a simple request that succeeds without CORS headers for the _server_ but fails for _you_; then add the headers and fix it. Document the preflight for a `POST` with `content-type: application/json`.
