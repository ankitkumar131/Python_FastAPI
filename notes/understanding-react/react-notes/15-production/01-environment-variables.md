...
💡 **The standard fix for "we need the key in the browser"** is a small server-side proxy: the browser calls _your_ endpoint, your server attaches the privileged key, and the browser never holds it. It also gives you a place to rate-limit and log, which is what stops a leaked client key from becoming an invoice.
...
| 11 | Using env vars for feature flags _and_ permissions | two concepts tangled                                   | flags are config; permissions come from the session (Part 14) |
...
2. `DB_PASSWORD=super-secret-do-not-ship` in `.env`; `grep -r "super-secret" dist/` finds nothing in either build (measured above). Note the caveat: that only means it is not in the _bundle_ — the file is still in the repo, so it is not a secret at all.
