...
1. The dynamic import creates a separate chunk (`lazy-panel-DV0C4Lcy.js`), and the entry chunk is unchanged — proof that the module is not in the initial download. If the chunk did _not_ appear, the import would be static somewhere (or the bundler inlined it).
