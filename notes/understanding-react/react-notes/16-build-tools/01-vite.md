...
That warning is worth remembering twice: first as a fact about the toolchain (check the version a tutorial was written for), and second as the reason Part 15, file 05 measured the _actual_ way to strip `console` calls in Vite 8 (a minifier setting, not an esbuild one).
