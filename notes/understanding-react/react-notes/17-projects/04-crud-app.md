...
Why this project: create / read / update / delete is the shape of most business software, and it is where three chapters finally meet — routing (Part 6), forms (Part 7/8) and server state (Part 9). The interesting questions are all about _agreement_: after a create, does the list show the new item? After an edit, is the detail page fresh? If a validation rule lives in the form, does the API layer respect it too? This project answers them with a small library app — books, five endpoints, four screens — and eight tests that walk the lifecycle.

Measured: the finished project's tests run in **817 ms** (8 tests) in this lab's suite — `/home/user/lab/part17-projects.txt`.
...
⚠️ **Client validation is a courtesy, not a security control** (the rule from Part 15, file 06, applied here): the API must validate too, because anybody can `curl` the endpoint. The reason to have it _in the client at all_ is UX — the user learns about the problem instantly, and the network is not troubled with data that cannot be accepted.
...
| `setQueryData` **and** `invalidateQueries` | the first makes the UI correct immediately; the second makes it correct _according to the server_ (a form of optimistic update with a safety net) |
...
| 2  | Mutating and _not_ invalidating                             | the list shows yesterday's data after a create | `setQueryData` + `invalidateQueries`                            |
...
* **CRUD is four screens and five endpoints**, and the hard part is _agreement_: after every write, the UI must show what the server considers true.
