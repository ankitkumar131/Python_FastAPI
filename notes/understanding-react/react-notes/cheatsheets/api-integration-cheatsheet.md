...
> **Part 18 · Reference · Cheatsheet 6 of 9** Everything below assumes one rule: **one module knows the URL, one hook per operation, components never see `fetch`.**
...
Validation library options: Zod (schema drives the type), Valibot (smaller), io-ts, ArkType. Whichever you choose, the value of the work is the _failure mode_: a clear error at the boundary instead of `undefined°` in the UI.
