...
The same is true of axios: `api.get<Product[]>(…)` types `response.data` as `Product[]`, but that is _also_ just an assertion — axios does no parsing (file 03, section 3). The library's generic is a comment with grammar.
...
| ---------------------------------- | ---------------------------------------------------------------------------------------- | ----------------------------------- | --------------------------------- |
| **Compile-time types**             | mistakes in _our_ code                                                                   | `src/api/types.ts`, component props | `tsc -b`                          |
...
Verified in file 07: the lab server stored `"9.99"` in a field that should be an integer, because it has no request validation. That single fact proves the point — a permissive server means the _client_ must also validate what comes back, or the corruption spreads into the UI.
...
| `blurb !== null && typeof blurb !== 'string'`     | `null` is _allowed_; `undefined` and numbers are not                                                  |
...
| **Generated types** (OpenAPI, GraphQL codegen) | depends                            | the types _are_ the API contract                                                                                                        | only as good as the spec; still needs runtime validation for trust boundaries          |
...
**(a) `@ts-expect-error` as a test.** Verified in the probe — five assertions that a _wrong_ program still fails to compile:
...
1. In a scratch file, do this and observe what TypeScript does _not_ catch:
...
4. Add a contract probe that exercises every endpoint (list, detail, `404`, `422`, `500`, a bad token) and prints one `PASS`/`FAIL` line per case, including the case where the payload is _deliberately_ malformed.
...
4.  ```ts
    ...
    `[]` parses to `[]`; `[null]` throws with `item 0: expected an object` — which tells you _which_ element is broken instead of just "one of them".
...
* **A cast is a promise, not a check.** `as Product` and `api.get<Product>()` assert; a parser _verifies_.
* **DTO ≠ domain model.** Keep snake\_case, `0 | 1` booleans and optional fields at the edge, and map them once (verified: `price_minor → priceMinor`, `in_stock: 0 → inStock: false`, missing `blurb → null`).
