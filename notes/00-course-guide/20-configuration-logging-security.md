...
[Previous](19-project-architecture.md) · [Course map](./) · [Next](21-docker-and-deployment.md)
...
Default a non-secret display name; CATALOG\_APP\_NAME overrides it.

### Line 12

Parse textual boolean settings correctly, unlike bool(nonempty\_string).
...
`cp` copies the template to the explicitly loaded local filename. In PowerShell use `Copy-Item .env.example .env`. Do not overwrite a real existing .env without preserving its values. After startup `/info` returns the configured name and page\_limit, never the optional secret.
