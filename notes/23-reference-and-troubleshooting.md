...
[Previous](22-performance-and-operations.md) · [Course map](00-course-guide.md) · [Advanced labs](24-advanced-pattern-labs.md)
...
| [Pydantic: v2 migration](https://docs.pydantic.dev/latest/migration/) | model_dump/model_validate, validator changes, from_attributes and required/nullable semantics; chapter 05 |
...
| [SQLAlchemy 2 ORM quick start](https://docs.sqlalchemy.org/en/20/orm/quickstart.html) | DeclarativeBase, Mapped, mapped_column and select-based queries; chapters 11/22 |
...
| BaseSettings imported from pydantic | Import from pydantic_settings |
...
| 404 despite successful startup | Path/method/router not registered | Inspect `/docs` and verify include_router; `/` is not automatically every resource |
...
| Missing JWT_SECRET startup error | Configuration intentionally fails closed | Generate a local high-entropy value and provide it to the process; never add a weak fallback |
...
| `.env` changes do nothing | Not loaded or cached settings | Confirm SettingsConfigDict env_file, cwd/prefix, environment precedence, then restart/clear test cache |
...
| Container cannot write SQLite | Wrong volume permissions/path | Use the owned `/data` mount and absolute DATABASE_URL from the Dockerfile |
